...
open T, '+</dev/cu.usbmodem1001' or die($!);
select T; $| = 1;
select STDOUT; $| = 1;
...
# Teensy++ 1.0 or 2.0
if ($version eq 'at90usb1286' or $version eq 'at90usb646') {
...
# pulse each pin, in order, over and over
# good for looking for shorts with a scope

for (@all) { putz $_, "0oi" }
while(1) {
	for (@all) { putz $_, "1o0oi" }
}
